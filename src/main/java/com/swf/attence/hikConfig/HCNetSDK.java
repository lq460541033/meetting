...
//EMAIL功能(NET_DVR_EMAILCFG_V30结构)
    ...
    public static final int NET_DVR_GET_NETCFG_V50  =   1015;    //获取网络参数配置(V50)
    public static final int NET_DVR_SET_NETCFG_V50  =   1016;    //设置网络参数配置(V50)
...
//设备编码类型配置(NET_DVR_COMPRESSION_AUDIO结构)
    ...
    public static final int NET_DVR_GET_USERCFG_V50  =  6246;  //获取用户参数V50
    public static final int NET_DVR_SET_USERCFG_V50  =  6247;  //设置用户参数V50
    /***************************DS9000新增命令(_V30) end *****************************/
    /*************************参数配置命令 end*******************************/
    /*******************查找文件和日志函数返回值*************************/
...
public static class NET_DVR_USER_V50 extends Structure
{
    public int dwSize;  //结构体大小
    public int dwMaxUserNum; //设备支持的最大用户数-只读
    public NET_DVR_USER_INFO_V40[] struUser = new NET_DVR_USER_INFO_V40[MAX_USERNUM_V30];  /* 用户参数 */
    public byte[]    sloginPassword = new byte[PASSWD_LEN];          /* 登陆密码确认 */
    public byte[]     byRes = new byte[240]; //保留
}

//单用户参数(子结构)(扩展)
public static class NET_DVR_USER_INFO_V40 extends Structure
{
    public byte[]    sUserName = new byte[NAME_LEN];            /* 用户名只能用16字节 */
    public byte[]    sPassword = new byte[PASSWD_LEN];            /* 密码 */
    public byte[]    byLocalRight = new byte[MAX_RIGHT];    /* 本地权限 */
    /*数组0: 本地控制云台*/
    /*数组1: 本地手动录象*/
    ...
    /*数组12: 远程管理模拟和IP camera */
    /*数组13: 远程关机/重启 */
    public int[]    dwNetPreviewRight = new int[MAX_CHANNUM_V40];            /* 远程可以预览的通道，从前往后顺序排列，遇到0xffffffff后续均为无效*/
    public int[]    dwLocalRecordRight = new int[MAX_CHANNUM_V40];            /* 本地可以录像的通道，从前往后顺序排列，遇到0xffffffff后续均为无效*/
    public int[]    dwNetRecordRight = new int[MAX_CHANNUM_V40];            /* 远程可以录像的通道，从前往后顺序排列，遇到0xffffffff后续均为无效*/
    public int[]    dwLocalPlaybackRight = new int[MAX_CHANNUM_V40];            /* 本地可以回放的通道，从前往后顺序排列，遇到0xffffffff后续均为无效*/
    public int[]    dwNetPlaybackRight = new int[MAX_CHANNUM_V40];            /* 远程可以回放的通道，从前往后顺序排列，遇到0xffffffff后续均为无效*/
    public int[]    dwLocalPTZRight = new int[MAX_CHANNUM_V40];                /* 本地可以PTZ的通道，从前往后顺序排列，遇到0xffffffff后续均为无效*/
    public int[]    dwNetPTZRight = new int[MAX_CHANNUM_V40];                /* 远程可以PTZ的通道，从前往后顺序排列，遇到0xffffffff后续均为无效*/
    public int[]    dwLocalBackupRight = new int[MAX_CHANNUM_V40];            /* 本地备份权限通道，从前往后顺序排列，遇到0xffffffff后续均为无效*/
    public NET_DVR_IPADDR    struUserIP;                /* 用户IP地址(为0时表示允许任何地址) */
    public byte[]     byMACAddr = new byte[MACADDR_LEN];    /* 物理地址 */
    public byte     byPriority;                /* 优先级，0xff-无，0--低，1--中，2--高 */
                                        /* 无……表示不支持优先级的设置
    ...
    public byte[]    byRes1 = new byte[2];               //四字节对齐
    public byte[]    byPublishRight = new byte[MAX_RIGHT];  //信息发布专有权限
    /*数组0: 素材审核权限*/
    /*数组1: 节目审核权限*/
    /*数组2: 日程审核权限*/
    /*数组3: 上传素材权限*/
...
public static class NET_DVR_ACS_WORK_STATUS extends Structure {
	public int dwSize;
	public byte[] byDoorLockStatus = new byte[MAX_DOOR_NUM]; //门锁状态，0-关，1-开
	public byte[] byDoorStatus = new byte[MAX_DOOR_NUM]; //门状态，1-休眠，2-常开状态，3-常闭状态，4-普通状态
	public byte[] byMagneticStatus = new byte[MAX_DOOR_NUM]; //门磁状态，0-闭合，1-开启
	...
	public byte[] byCardReaderOnlineStatus = new byte[MAX_CARD_READER_NUM]; //读卡器在线状态，0-不在线，1-在线
	public byte[] byCardReaderAntiDismantleStatus = new byte[MAX_CARD_READER_NUM]; //读卡器防拆状态，0-关闭，1-开启
	public byte[] byCardReaderVerifyMode = new byte[MAX_CARD_READER_NUM]; //读卡器当前验证方式，1-刷卡，2-刷卡+密码，3-刷卡，4-刷卡或密码
	public byte[] bySetupAlarmStatus = new byte[MAX_ALARMHOST_ALARMIN_NUM];//报警输入口布防状态，0-对应报警输入口处于撤防状态，1-对应报警输入口处于布防状态
	public byte[] byAlarmInStatus = new byte[MAX_ALARMHOST_ALARMIN_NUM]; //报警输入口报警状态，0-对应报警输入口当前无报警，1-对应报警输入口当前有报警
	public byte[] byAlarmOutStatus = new byte[MAX_ALARMHOST_ALARMOUT_NUM]; //报警输出口状态，0-对应报警输出口无报警，1-对应报警输出口有报警
...
public static class NET_VCA_FACESNAP_RESULT  extends Structure {
	...
	public byte[] byRes2 = new byte[7];              // 保留字节
...
public static class NET_VCA_FACESNAP_MATCH_ALARM extends Structure {
	...
	public byte byLivenessDetectionStatus;//活体检测状态
	public byte[] byRes = new byte[2];              // 保留字节
